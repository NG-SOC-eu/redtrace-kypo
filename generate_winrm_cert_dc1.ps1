# ======================================================
# WinRM HTTPS Certificate Generator "dc1", "172.25.0.40"
# ======================================================

Write-Host "`nRemoving previous certificate CN=dc1 created" -ForegroundColor Cyan

Get-ChildItem Cert:\LocalMachine\My | Where-Object { $_.Subject -like "*CN=dc1*" } | ForEach-Object {
    Remove-Item -Path "Cert:\LocalMachine\My\$($_.Thumbprint)" -Force
    Write-Host "Removed: $($_.Thumbprint)" -ForegroundColor Green
}

Write-Host "`nGenerating certificate with IP: 172.25.0.40" -ForegroundColor Cyan

# Generate the self-signed certificate
$cert = New-SelfSignedCertificate `
  -DnsName "dc1", "172.25.0.40" `
...
winrm create winrm/config/Listener?Address=*+Transport=HTTPS "@{Hostname=`"dc1`";CertificateThumbprint=`"$thumb`"}"
